...
void Graph::InsertPlaceholder(const string &node_name) {
    ...
    Node *temp=new PlaceholderNode(node_name);
    nodes_.insert(node_name,temp);
}

void Graph::InsertConstant(const string &node_name, float value) {
    // wait to fill, 使用nodes_.insert()新建一个常量
    Node *temp=new ConstantNode(node_name,value);
    nodes_.insert(node_name,temp);
}
...
void Graph::InsertOperation(const string &node_name, const string &operation_type, 
    ...
    if (operation_type=="+"){
        Node *temp=new Plus(node_name,nodes_[input1],nodes_[input2]);
        nodes_.insert(node_name,temp);
    } 
    else{
        Node *temp=new Minus(node_name,nodes_[input1],nodes_[input2]);
        nodes_.insert(node_name,temp);
    }
...
void Graph::Eval(const string &node_name, const int &argument_num, 
    ...
    for(int i=0;i<argument_num;++i){
        Node *temp1=nodes_[argument_name_list_[i]];
        PlaceholderNode *temp2=dynamic_cast<PlaceholderNode*>(temp1);
        temp2->set_val(argument_value_list_[i]);
    }
